...
export const parseGameState = gameState => {
    return JSON.parse(
        pako.inflate(atob(gameState.substring(32)), { to: 'string' }),
    );
};
...
const aggregateBonuses = relics => {
    return [].concat(...relics.map(r => r.bonuses)).reduce((agg, b) => {
        if (!agg[b.id]) {
            agg[b.id] = { ...b };
        } else {
...
const relicBonusCallbacks = {
    3: (relicLevels, totalLevels, xyliqilLevel, ponyboyLevel) => {
        const totalSolomonBonus = solomonBonusCallback(
            totalLevels,
            ponyboyLevel,
        );
        const diffSolomonBonus = solomonBonusCallback(
            totalLevels - relicLevels,
            ponyboyLevel,
        );
    ...
    4: (relicLevels, totalLevels, xyliqilLevel) => {
        ...
        const diffBonus = idleAncientBonusCallback(
            totalLevels - relicLevels,
            xyliqilLevel,
        );
    ...
    11: (relicLevels, totalLevels) => {
        const totalBonus =
            100 * (1 - Math.exp(-0.01 * (totalLevels + relicLevels)));
    ...
    12: (relicLevels, totalLevels) => {
        ...
        const diffBonus =
            100 * (1 - Math.exp(-0.0025 * (totalLevels - relicLevels)));
    ...
    13: (relicLevels, totalLevels) => {
        ...
        const diffBonus =
            75 * (1 - Math.exp(-0.013 * (totalLevels - relicLevels)));
    ...
    14: (relicLevels, totalLevels) => {
        ...
        const diffBonus =
            9900 * (1 - Math.exp(-0.002 * (totalLevels - relicLevels)));
    ...
    17: (relicLevels, totalLevels) => {
        ...
        const diffBonus =
            30 * (1 - Math.exp(-0.034 * (totalLevels - relicLevels)));
    ...
    18: (relicLevels, totalLevels) => {
        ...
        const diffBonus =
            50 * (1 - Math.exp(-0.02 * (totalLevels - relicLevels)));
    ...
    20: (relicLevels, totalLevels) => {
        ...
        const diffBonus =
            75 * (1 - Math.exp(-0.026 * (totalLevels - relicLevels)));
    ...
    21: (relicLevels, totalLevels) => {
        ...
        const diffBonus =
            8 * (1 - Math.exp(-0.01 * (totalLevels - relicLevels)));
...
export const getRelics = state => {
    ...
    const relics = Object.keys(items)
        .map(k => {
            const item = items[k];
            let total = 0;
    ...
    relics.forEach(r => {
        r.bonuses.forEach(b => {
            ...
            const totalLevels =
                ancient.baseLevel + (ancientBonuses ? ancientBonuses.level : 0);

            b.tooltip = ancient.relicText.replace(
                '{}',
                roundNum(
                    relicBonusCallbacks[b.id](
                        b.level,
                        totalLevels,
                        xyliqilLevel,
                        ponyboyLevel,
